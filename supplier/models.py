...
class Country(models.Model):
    ...
    mode = models.CharField(
        max_length=2,
        verbose_name="Режим ВЭД",
        choices=[("a", "благоприятный режим"), ("b", "особый режим")],
    )
...
class Supplier(models.Model):
    ...
    data_source = models.CharField(
        max_length=255, verbose_name="Источник данных", blank=True, null=True
    )
    tn_ved = models.CharField(
        max_length=50, verbose_name="Код ТН ВЭД", blank=True, null=True
    )
    price = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name="Цена", default=10
    )
    price_date = models.DateField(
        verbose_name="Дата цены", help_text="2022-08-01", blank=True, null=True
    )
    created_date = models.DateField(auto_now_add=True, verbose_name="Создан", null=True)
    updated_date = models.DateField(auto_now=True, verbose_name="Изменен", null=True)
...
class Logistic(models.Model):
    ...
    product = models.TextField(
        db_index=False, verbose_name="Логистическая услуга (английский)"
    )
    contact = models.TextField(verbose_name="Контактная информация")
    description_ru = models.TextField(verbose_name="Описание (русский)")
    product_ru = models.TextField(
        db_index=False, verbose_name="Логистическая услуга (русский)"
    )
    email = models.EmailField(max_length=254)
    data_source = models.CharField(
        max_length=255, verbose_name="Источник данных", blank=True, null=True
